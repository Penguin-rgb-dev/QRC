...
# version 0: uses 1. shifting and squashing of the original series; 2. input map s in [-1,1]; 3. no normalization of x_features to [0,1]; 4. cliping pred value to [-1,1].
...
# shifting and squashing: y --> tanh(y-1)
s_raw = np.tanh(s_raw - 1)
...
y_target_84 = np.arctanh(s_test[:,-1])+1
...
def input_map(rho_in, s, N):
    ...
    psi_s = np.array([np.sqrt(1+s), np.sqrt(1-s)], dtype=complex)/np.sqrt(2)
...
for i in range(num_test_seq):
    ...
    x_features = np.real(obs_matrix @ rho.flatten())
...
output_file = os.path.join(output_dir, f"nrmse_84_v0_{seed}.npz")
np.savez_compressed(
    output_file,
    nrmse = nrmse,
