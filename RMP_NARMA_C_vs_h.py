...
for i in range(0, total_W, chunk_size):
    ...
    checkpoint_name = f"NARMA_checkpoint_W_index_{i}.npz"
...
if len(results_flat) == len(W_values) * n_realizations:
    ...
    print(f"Warning: Simulation ended early. Collected {len(results_flat)}/{len(W_values)*n_realizations} results.")
    # Optional: save whatever we managed to get
    np.savez_compressed('NARMA_PARTIAL_Final_Results.npz', data=results_flat)
